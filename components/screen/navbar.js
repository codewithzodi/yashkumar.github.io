		...
		return (
			<div className="main-navbar-vp absolute top-0 right-0 w-screen shadow-md flex flex-nowrap justify-between items-center bg-ub-grey text-ubt-grey text-sm select-none z-50">
				...
				<div
					...
					className={
						'absolute left-1/2 transform -translate-x-1/2 cursor-pointer pl-2 pr-2 text-xs md:text-sm outline-none transition duration-100 ease-in-out border-b-2 border-transparent focus:border-ubb-orange py-1'
					}
				>
					<Clock />
					<CalendarCard 
						...
						toggleVisible={() => {
							this.setState({ calendar_card: false });
						}}
					/>
				</div>
				<div
					...
					className={
						'relative pr-3 pl-3 outline-none transition duration-100 ease-in-out py-1 group flex items-center'
					}
				>
					<Status />
					<StatusCard
